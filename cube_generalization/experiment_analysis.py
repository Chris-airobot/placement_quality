...
def sweep_thresholds(y, p):
    ...
    best_bal, best_f1 = (0.0, 0.5), (0.0, 0.5)  # (score, tau)
    for t in ths:
        yhat = (p >= t).astype(int)  # predict collision if p_collision >= t
        bal  = balanced_accuracy_score(y, yhat)
        f1   = f1_score(y, yhat, zero_division=0)
        if bal > best_bal[0]: best_bal = (bal, t)
        if f1  > best_f1[0]:  best_f1  = (f1,  t)
    return best_bal, best_f1
...
def eval_file(path, title):
    ...
    (bal, tau_bal), (f1s, tau_f1) = sweep_thresholds(y, p)
