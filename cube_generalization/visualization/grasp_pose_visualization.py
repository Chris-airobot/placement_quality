...
from utils import sample_object_poses
from experiment_generation import generate_grasps
...
class GraspVisualizer(Node):
    def __init__(self):
        ...
        # self.box_dims = np.random.uniform(0.05, 0.2, size=3) 
        self.box_dims = np.array([0.05, 0.12, 0.2])

        # grasps_file = "/home/chris/Chris/placement_ws/src/placement_quality/docker_files/ros_ws/src/grasp_generation/test.json"
    
        
        self.all_object_poses = sample_object_poses(12, self.box_dims)
        ...
        # This pose is normally provided by your simulator (or set randomly)
        total_object_poses = len(self.all_object_poses)
        print(f"The total number of object poses is: {total_object_poses}")
        random_index = np.random.randint(0, total_object_poses)
        random_index = 1
        self.object_pose = self.all_object_poses[random_index]
        self.object_pose[2] += 0.20
        print(f"The object pose is: {self.object_pose}")
        self.grasp_poses = generate_grasps(self.object_pose[:3], self.object_pose[3:], self.box_dims, 
                                           enable_yaw_tilt=True)


        self.transformed_grasp_pose = None
        # Index of the grasp currently being visualised
        self.current_grasp_idx = 0

        # Timer will be created later (after the world and target are ready)
        self.grasp_timer = None
        self.grasp_offset = [0, 0, -0.08]
    ...
    def setup(self):
        ...
        self.create_object(self.object_pose[:3], 
                           self.object_pose[3:])
        ...
        # Apply the first grasp and start a timer to cycle through all grasps.
        self.apply_grasp(self.current_grasp_idx)

        # One-second timer to update to the next grasp.
        self.grasp_timer = self.create_timer(1.0, self.update_grasp_pose)
    ...
    def apply_grasp(self, grasp_idx):
        """Set the target (and visualisation) to the grasp at the given index."""
        self.world_grasp_pose = self.grasp_poses[grasp_idx]

        # Compute offset-corrected pose (local frame offset)
        self.transformed_grasp_pose = local_transform(self.world_grasp_pose, self.grasp_offset)

        # Optionally draw the frame for debugging
        # draw_frame(self.world_grasp_pose[:3], self.world_grasp_pose[3:])

        # Move target frame to the grasp pose
        self._target.set_world_pose(self.transformed_grasp_pose[:3], self.world_grasp_pose[3:])

    def update_grasp_pose(self):
        """Timer callback: advance to the next grasp every second."""
        self.current_grasp_idx = (self.current_grasp_idx + 1) % len(self.grasp_poses)
        self.apply_grasp(self.current_grasp_idx)
